...
services.forEach(service => {
  ...
  try {
    ...
    // 2. Generate XML report AND ENFORCE 25% PER FILE inside container
    console.log(`Generating XML report and checking thresholds (25% per file) in ${reportDir}...`);
    try {
      execSync(`docker exec ${service.container} mvn jacoco:report jacoco:check -Pe2e-check`, { stdio: 'inherit' });
    } catch (checkError) {
      console.error(`\n[!] Threshold check FAILED for ${service.name}. Some files have < 25% E2E coverage.`);
      allPassed = false;
    }
...
if (!allPassed) {
  console.log('\n[!] Backend E2E coverage failed to meet quality standards (25% per file).');
  process.exit(1);
}

console.log('\nBackend E2E coverage processing complete. All files >= 25% coverage.');
