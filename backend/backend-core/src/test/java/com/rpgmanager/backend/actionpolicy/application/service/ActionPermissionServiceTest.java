...
class ActionPermissionServiceTest {
    ...
    void shouldReturnFalseWhenBlockedGloballyInCampaignAndNoFurtherOverrides() {
        // given
        when(actionPolicyRepositoryPort.findByActionTypeAndContextTypeAndContextId(ActionType.LEVEL_UP,
                ContextType.CAMPAIGN, CAMPAIGN_ID))
                .thenReturn(Optional.of(ActionPolicy.builder().isAllowed(false).build()));

        // when
        boolean result = underTest.canPerformAction(CHARACTER_ID, ActionType.LEVEL_UP, CAMPAIGN_ID, null, null);
    ...
    void shouldReturnFalseWhenAllowedInCampaignButDeniedInSession() {
        ...
        // when
        boolean result = underTest.canPerformAction(CHARACTER_ID, ActionType.LEVEL_UP, CAMPAIGN_ID, SESSION_ID, null);
