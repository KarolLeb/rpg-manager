...
class AdminPolicyManagementControllerTest {
  ...
  @Mock private JpaActionPolicyRepository actionPolicyRepository;

  @Mock private JpaCharacterActionOverrideRepository characterActionOverrideRepository;

  @InjectMocks private AdminPolicyManagementController controller;
  ...
  void setPolicy_shouldUpdateAllowedAndSave() {
    ...
    ResponseEntity<Void> response =
        controller.setPolicy(ActionType.LEVEL_UP, ContextType.CAMPAIGN, 1L, true);
  ...
  void setCharacterOverride_shouldUpdateAllowedAndSave() {
    ...
    ResponseEntity<Void> response =
        controller.setCharacterOverride(100L, ActionType.LEVEL_UP, ContextType.SESSION, 2L, false);

    assertThat(response.getStatusCode()).isEqualTo(HttpStatus.CREATED);
    ArgumentCaptor<CharacterActionOverrideEntity> captor =
        ArgumentCaptor.forClass(CharacterActionOverrideEntity.class);
