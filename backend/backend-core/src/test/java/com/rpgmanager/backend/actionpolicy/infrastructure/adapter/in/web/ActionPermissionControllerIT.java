...
@AutoConfigureMockMvc(addFilters = false) // Add filters false bypasses Spring Security for this unit-level controller
                                          // test
class ActionPermissionControllerIT {
        ...
        void shouldReturnTrueWhenServiceAllowsAction() throws Exception {
                // given
                when(actionPermissionService.canPerformAction(eq(1L), eq(ActionType.LEVEL_UP), any(), any(), any()))
                                .thenReturn(true);

                // when & then
                mockMvc.perform(get("/api/v1/permissions/check")
                                .param("characterId", "1")
        ...
        void shouldReturnFalseWhenServiceDeniesAction() throws Exception {
                // given
                when(actionPermissionService.canPerformAction(eq(2L), eq(ActionType.DISTRIBUTE_POINTS), eq(100L), any(),
                                any()))
                                .thenReturn(false);
